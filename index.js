...
function playGame() {
  ...
  rl.question("\nเดินไปกัดเลยครับ! ( w | a | s | d ): ", (input) => {
    ...
    // กำหนดให้เกมจบถ้าตกขอบ ใช้ if ง่ายๆไปเลย
    if (
      newY < 0 || // ตำแหน่งใหม่เลขที่ได้น้อยกว่า 0 ตกขอบคำสั่งทำงาน
      newY >= field.length || // ตำแหน่งใหม่เลขมากว่าจำนวนของ map ตาย
      // x ก็ทำเหมือนกัน
      newX < 0 ||
      newX >= field[0].length
    ) {
      //ส่วนนี้คือคำสี่งที่แสดงออกมา วีนอีก
      console.log("\n💀 วูบ! เดินออกไปไหนล่ะนั้น");
      //แสดงเสร็จล้มโต๊ะ
      rl.close();
      return;
    }
    ...
    //ต่อมาผมจับตำแหน่งของแมงมุมว่าปัจจุบันอยู่ไส
    const nowPosition = field[spiderY][spiderX];

    // กำหนดขอบเขตกันจ้า (ใกล้เสร็จแล้วววว)
    // อันแรกคือโดนระเบิดครับ ถ้าเลขตำแหน่งปัจจุบันมันเท่ากับตำแหน่งระเบิดให้แสดงข้อความ
    if (nowPosition === bomb) {
      console.log("\n💥 ตู้ม!!! เป็นโกโก้ครั้น"); //ข้อความนี้
      //แสดงเสร็จล้มโต๊ะ
      rl.close();
      return;
    }

    // อัน 2 คือรูปแบบเดียวกัน แต่เปลี่ยนเป็นตำแหน่งปีเตอร์
    if (nowPosition === peter) {
      console.log("\n🧬 กัดแบบง้างสุดแรงเกิด กำเนิดสไปเดอร์แมน!! 🧬"); //สรุปปีเตอร์คือใครนะครับ
      //แสดงเสร็จล้มโต๊ะเหมือนเดิม
      rl.close();
      return;
    }
